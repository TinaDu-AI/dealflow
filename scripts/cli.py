...
def cmd_my_profile(args: argparse.Namespace) -> None:
    """获取当前登录用户的主页（无需传 user-id）。"""
    import json as _json
    import re

    from xhs.urls import EXPLORE_URL
    from xhs.user_profile import _extract_user_profile_data
    ...
    try:
        ...
        from xhs.selectors import USER_PROFILE_NAV_LINK

        profile_href = page.evaluate(
            f"document.querySelector({_json.dumps(USER_PROFILE_NAV_LINK)})?.getAttribute('href') || ''"
        )
        if not profile_href:
            _output({"success": False, "error": "未找到个人主页链接，请确认已登录"}, exit_code=1)
            return

        # href 格式: /user/profile/<user_id>?xsec_token=xxx
        match = re.search(r"/user/profile/([^/?]+)", profile_href)
        if not match:
            _output({"success": False, "error": f"无法解析用户ID: {profile_href}"}, exit_code=2)
            return
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # my-profile
    sub = subparsers.add_parser("my-profile", help="获取当前登录用户的主页")
